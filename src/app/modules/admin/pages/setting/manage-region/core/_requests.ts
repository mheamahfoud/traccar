...
import {  Region, RegionQueryResponse } from './_models'
const getList = (query: string, page: number): Promise<RegionQueryResponse> => {
  return axios
    .post(`all_region?${'page=' + page}`, {
...
const create = (object: Region) => {
  return axios
    .post('store_region', object)
    .then((response: AxiosResponse<ResponeApiCheck>) => response.data)
  //.then((response: Response<VehicleType>) => response.data)
}

const update = (object: Region) => {
  return axios.post(`update_region/${object.parent}`, object)
    .then((response: AxiosResponse<ResponeApiCheck>) => response.data)
