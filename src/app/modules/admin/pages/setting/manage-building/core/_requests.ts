...
const getList = (query: string, page: number): Promise<BuildingQueryResponse> => {
  return axios
    .post(`all_building?${'page=' + page}`, {
...
const create = (object: Building) => {
  return axios
    .post('store_building', object)
    .then((response: AxiosResponse<ResponeApiCheck>) => response.data)
  //.then((response: Response<VehicleType>) => response.data)
}

const update = (object: Building) => {
  return axios.post(`update_building/${object.parent}`, object)
    .then((response: AxiosResponse<ResponeApiCheck>) => response.data)
...
const destroy = (id: ID): Promise<void> => {
  return axios.post(`${'update_building'}/${id}`).then(() => { })
}

const destroySelectedItems = (selectedIds: Array<ID>): Promise<void> => {
  const requests = selectedIds.map((id) => axios.post(`${'update_building'}/${id}`))
